...
// import 'solo_adventure_memory_screen.dart'; 

// class SoloMap extends StatefulWidget {
//   const SoloMap({super.key});

//   @override
//   State<SoloMap> createState() => _SoloMapState();
// }

// class _SoloMapState extends State<SoloMap> {
//   final ScrollController _scrollController = ScrollController();
//   final int totalNodes = 30; 
...
//   StreamSubscription? _soloProgressSubscription;
...
//       _soloProgressSubscription = FirebaseFirestore.instance.collection('solo_progress').doc(myUid).snapshots().listen((snapshot) async {
//         if (snapshot.exists) {
...
//           final authProvider = Provider.of<AuthProvider>(context, listen: false);
//           final myUid = authProvider.user!.uid;
//           await _generateNextNode(myUid);
//         }
//       }
//     }
//   }

//   Future<void> _fetchRatings(String myUid) async {
//     if (_adventurePath.isEmpty) return;
//     Map<int, double> tempRatings = {};
    
//     List<String> docIds = _adventurePath.map((id) => '${myUid}_$id').toList();
    
//     // Fetch en lotes si son más de 30 (límite de Firestore para 'whereIn')
//     for (var i = 0; i < docIds.length; i += 30) {
//       var subList = docIds.sublist(i, i + 30 > docIds.length ? docIds.length : i + 30);
//       var snapshot = await FirebaseFirestore.instance.collection('solo_memories').where(FieldPath.documentId, whereIn: subList).get();
...
//     if (mounted) setState(() => _adventureRatings = tempRatings);
//   }

//   Future<void> _generateNextNode(String myUid) async {
...
//     await FirebaseFirestore.instance.collection('solo_progress').doc(myUid).update({
...
//     _soloProgressSubscription?.cancel();
//     super.dispose();
//   }
...
//     return List.generate(30, (i) => Offset(mapWidth * (0.05 + (i * 0.23) % 0.9), mapHeight - (i * 187) % mapHeight));
//   }

//     void _showAdventureDetail(Map<String, dynamic> adventure, int nodeIndex) {
//     // ✅ CALCULAMOS LAS AVENTURAS DISPONIBLES AQUÍ
...
//                       Center(child: Text(adventure['title'] ?? '', textAlign: TextAlign.center, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Color(0xFF1976D2)))),
...
//                           },
//                           style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1976D2), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20))),
...
//     // ✅ VOLVEMOS A CALCULAR LAS AVENTURAS DISPONIBLES
//     List<int> availableIds = _adventuresCache.keys.where((id) => !_adventurePath.contains(id)).toList();
...
//               children: [
//                 const Icon(Icons.lightbulb_outline, color: Color(0xFF1976D2), size: 50),
//                 const SizedBox(height: 15),
...
//                         if (mounted) {
//                           // ✅ AQUÍ LE PASAMOS LA LISTA CORRECTA
//                           Navigator.push(context, MaterialPageRoute(builder: (_) => AdventureInProgressScreen(adventureData: adventure, availableAdventuresIds: availableIds, isSoloMode: true)));
//                         }
//                       },
...
//       });
//     } catch (e) {
//       debugPrint('Error guardando estado de cita solitaria: $e');
//     }
//   }
...
//         children: [
//           Container(
//             decoration: const BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0xFF64B5F6), Color(0xFFE3F2FD), Color(0xFFAED581), Color(0xFF66BB6A)], stops: [0.0, 0.3, 0.6, 1.0])),
//             child: SingleChildScrollView(
...
//                   children: [
//                     ...ambientPoints.map((pos) => Positioned(left: pos.dx, top: pos.dy, child: Icon(Icons.local_florist, color: Colors.blue.withValues(alpha: 0.3), size: 30))),
//                     CustomPaint(size: Size(mapWidth, mapHeight), painter: CandyPathPainter(points: pathPoints, pathColor: const Color(0xFF64B5F6))), // Un poco más azul
...
//                       Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Mi Camino', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)), Text('$completedDates/$totalNodes', style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.bold, fontSize: 12))]),
//                       const SizedBox(height: 4),
//                       ClipRRect(borderRadius: BorderRadius.circular(10), child: LinearProgressIndicator(value: mapProgress, backgroundColor: Colors.white24, valueColor: const AlwaysStoppedAnimation<Color>(Color(0xFF1976D2)), minHeight: 8)),
//                     ]),
//                   ),
//                 ),
//               ],
...
//     bool isCompleted = isUnlocked && arrayIndex < _adventurePath.length - 1 && !isInProgress;
//     bool isNextStep = isUnlocked && arrayIndex == _adventurePath.length - 1 && !isInProgress;
...
//     } else if (isNextStep) { 
//       startColor = const Color(0xFF42A5F5); endColor = const Color(0xFF1976D2); 
...
//     return Positioned(
//       left: x - 30, top: y - (showRating ? 42 : 30), 
//       child: GestureDetector(
//         onTap: isLocked || adventureData == null ? null : () {
//           if (isInProgress) {
//             Navigator.push(context, MaterialPageRoute(builder: (_) => AdventureInProgressScreen(adventureData: adventureData, availableAdventuresIds: const [], isSoloMode: true)));
//           } else if (isCompleted) {
//             final myUid = Provider.of<AuthProvider>(context, listen: false).user!.uid;
...
//             Container(
//               width: 60, height: 60, 
//               decoration: BoxDecoration(shape: BoxShape.circle, gradient: RadialGradient(colors: [startColor, endColor], center: Alignment.center, radius: 0.5), border: Border.all(color: Colors.white, width: isInProgress ? 4 : 3), boxShadow: [BoxShadow(color: startColor.withValues(alpha: 0.6), blurRadius: isInProgress ? 12 : 6, offset: const Offset(2, 4))]),
//               child: Center(child: _isLoadingData ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : iconChild),
//             ),
//             if (showRating) Container(margin: const EdgeInsets.only(top: 4), padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10), boxShadow: const [BoxShadow(color: Colors.black26, blurRadius: 3, offset: Offset(0, 2))]), child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.star, color: Colors.amber, size: 14), const SizedBox(width: 2), Text(rating.toStringAsFixed(1), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.black87))])),
//           ],
//         ),
//       ),
//     );
